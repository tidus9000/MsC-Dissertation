...
#pragma warning disable 0414
    ...
    [DebugGUIPrint, DebugGUIGraph(group: 1, r: 1, g: 1, b: 1)]
    float m_timeToNextBeat = 0;
    //[DebugGUIPrint, DebugGUIGraph(group: 1, r: 0, g: 1, b: 0)]
    //float mouseY;

    [DebugGUIPrint, DebugGUIGraph(group: 0, min: 0, max: 30, r: 1, g: 1, b: 1)]
    float pickups;
    ...
    [DebugGUIPrint, DebugGUIGraph(group: 2, r: 0, g: 1, b: 1)]
    float cObj2;

    [DebugGUIPrint, DebugGUIGraph(group: 2, r: 1, g: 1, b: 1)]
    float cObj3;

    [DebugGUIPrint, DebugGUIGraph(group: 2, r: 1, g: 0, b: 0)]
    float cObj4;

    [DebugGUIPrint, DebugGUIGraph(group: 2, r: 1, g: 0, b: 1)]
    float cObj5;

    float[] m_nearestObjects;

    void Awake()
    {
        m_sm = GameObject.Find("Ship").GetComponent<Shipmovement>();
    ...
    void Update()
    {
        m_nearestObjects = m_sm.FindClosestObjects();

        if (m_nearestObjects.Length > 0)
        {
            cObj1 = m_nearestObjects[0];
        }

        if (m_nearestObjects.Length > 1)
        {
            cObj2 = m_nearestObjects[1];
        }

        if (m_nearestObjects.Length > 2)
        {
            cObj3 = m_nearestObjects[2];
        }

        if (m_nearestObjects.Length > 3)
        {
            cObj4 = m_nearestObjects[3];
        }

        if (m_nearestObjects.Length > 4)
        {
            cObj5 = m_nearestObjects[4];
        }

        // Update the fields our attributes are graphing
        //SinField = Mathf.Sin(Time.time * 6);
        m_horizontal = Input.GetAxis("Horizontal");
        m_score = m_gm.m_score;
        //mouseY = Input.mousePosition.y / Screen.height;

        pickups = m_gm.m_numberOfPickups;
        hazards = m_gm.m_numberOfHazards;
        missed = m_gm.missedCollectables;

        if (!m_gm.getPaused())
        {
            m_timeToNextBeat -= Time.deltaTime;
            if (m_timeToNextBeat <= 0 && m_gm.m_bpm != 0)
            {
                m_timeToNextBeat = 60 / m_gm.m_bpm;
            }
        }
        // Manual logging
